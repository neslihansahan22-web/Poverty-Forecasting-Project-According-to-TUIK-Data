...
TUIK dataflow ID: NA because the selected table is an istab table (tables_7)
...
The candidate methods were compared using standard performance measures calculated via empirical residuals ($e_t = Y_t - \hat{Y}_t$).
...
The complete accuracy comparison table is saved in: outputs/tables/accuracy_comparison.csv
...
The final forecast table is saved in: outputs/tables/final_forecast.csv
...
13. Clone or download the project repository.
14. Open poverty_forecasting_project.Rproj in RStudio.
15. Open forecasting_project.Rmd.
16. Click the "Knit" button in RStudio to render the notebook to HTML.
...
│   ├── accuracy_measures.R  
│   └── plots.R  
├── poverty_forecasting_project.Rproj  
└── .gitignore 
